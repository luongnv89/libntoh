...
void ntoh_send_to_tcp_segment(struct ip* iphdr){
	...
	if(size_payload>0){
		...
		ret = ntoh_tcp_add_segment(tcpsession,tcpstream,iphdr,size_total,(void *)iphdr);
	}else{
...
int main(int argc,char *argv[]){
	...
	while(( packet = pcap_next(handle,&header))!=0){
		...
		ntoh_send_to_tcp_segment((struct ip*)iphdrcp);
