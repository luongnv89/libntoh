/*
In this example, we are going to create a tcp session and add some tcp streams
compile: 
	gcc -o test ex2.c -lntoh -lpcap -lpthread
Run test and expected result:
./test -f ../data-set/googlefr-tcp-in-order.pcap 
[INFO] (ex2.c:59) libntoh version: 0.4a
[INFO] (ex2.c:163) New stream is added from:  192.168.0.45:48487
[INFO] (ex2.c:164) to: 64.233.166.94:80
[INFO] (ex2.c:27) Something is happening here..
...
void tcp_callback(pntoh_tcp_stream_t stream, pntoh_tcp_peer_t orig,pntoh_tcp_peer_t dest, pntoh_tcp_segment_t seg,int reason, int extra){
	log_info("Something is happening here..");
	return ;
}
